...
RC Table::rollback_insert(Trx *trx, const RID &rid) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to delete indexes of record(rid=%d.%d) while rollback "
              "insert, rc=%d:%s",
...
RC Table::make_record(int value_num, Value *values, char *&record_out) {
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (!value_cast(&value, field->type())) {
...
RC Table::scan_record(Trx *trx, ConditionFilter *filter, int limit,
                      std::function<RC(Record *record)> record_reader) {
  return scan_record(trx, filter, limit, (void *)&record_reader,
                     scan_record_invoke_function);
}
...
RC Table::update_record(Trx *trx, const char *attribute_name, Value *value,
                        int condition_num, Condition conditions[],
                        int *updated_count) {
  if (trx != nullptr) {
    LOG_WARN("Update transaction not supported yet, all changes cannot be "
             "rollbacked.");
    // return RC::GENERIC_ERROR;
  }
  ...
  if (!value_cast(value, field->type())) {
    LOG_ERROR("Got invaild value type: %d, expected: %d", value->type,
              field->type());
    return RC::SCHEMA_FIELD_TYPE_MISMATCH;
  }
  ...
  // Do update
  Record record;
  for (auto rid_it = matched_rids.begin(); rid_it != matched_rids.end();
       rid_it++) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_ERROR(
          "Failed to delete indexes of record for update (rid=%d.%d). rc=%d:%s",
          record.rid.page_num, record.rid.slot_num, rc, strrc(rc));
      return rc;
    }
    ...
    if (rc != RC::SUCCESS) {
      LOG_ERROR(
          "Failed to insert indexes of record for update (rid=%d.%d). rc=%d:%s",
          record.rid.page_num, record.rid.slot_num, rc, strrc(rc));
